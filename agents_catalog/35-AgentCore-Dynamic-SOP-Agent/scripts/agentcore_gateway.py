    ...
    try:
            ...
            "mcp": {
                "lambda": {
                    "lambdaArn": get_ssm_parameter(
                        "/app/myapp/agentcore/lambda_arn"
                    ),
        ...
        auth_config = {
            "customJWTAuthorizer": {
                "allowedClients": [
                    get_ssm_parameter(
                        "/app/myapp/agentcore/machine_client_id"
                    )
                ],
                "discoveryUrl": get_ssm_parameter(
                    "/app/myapp/agentcore/cognito_discovery_url"
                ),
            }
        }

        execution_role_arn = get_ssm_parameter(
            "/app/myapp/agentcore/gateway_iam_role"
        )
        ...
        put_ssm_parameter("/app/myapp/agentcore/gateway_name", gateway_name)
        put_ssm_parameter(
            "/app/myapp/agentcore/gateway_arn", create_response["gatewayArn"]
        )
        put_ssm_parameter(
            "/app/myapp/agentcore/gateway_url", create_response["gatewayUrl"]
        )
        put_ssm_parameter(
...
def get_gateway_id_from_config() -> str:
    ...
    try:
        return get_ssm_parameter("/app/myapp/agentcore/gateway_id")
    except Exception as e:
...
def delete(gateway_id, confirm):
    ...
    if delete_gateway(gateway_id):
        ...
        delete_ssm_parameter("/app/myapp/agentcore/gateway_id")
        delete_ssm_parameter("/app/myapp/agentcore/gateway_name")
        delete_ssm_parameter("/app/myapp/agentcore/gateway_arn")
        delete_ssm_parameter("/app/myapp/agentcore/gateway_url")
        delete_ssm_parameter("/app/myapp/agentcore/cognito_secret")
